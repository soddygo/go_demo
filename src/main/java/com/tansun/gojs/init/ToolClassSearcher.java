...
public class ToolClassSearcher {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ToolClassSearcher.class);
    ...
    private static <T> List<Class<? extends T>> extraction(Class<T> clazz, List<String> classFileList) {
        ...
        for (String classFile : classFileList) {
        	if(!classFile.startsWith("com.trs")){
        		continue;
        	}
            ...
            if (!baseDir.exists() || !baseDir.isDirectory()) {
                ...
                String[] filelist = baseDir.list(new FilenameFilter() {
                    ...
                    public boolean accept(File dir, String name) {
                        return includeAllJarsInLib || includeJars.contains(name) || name.startsWith("zzpt");
                    }
                });
                for (int i = 0; i < filelist.length; i++) {
